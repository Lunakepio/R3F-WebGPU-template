...
export function useMaterial() {
  ...
  const normalizedPosition = normalize(positionGeometry);
  const opacity = sub(1.0, abs(normalizedPosition.y)); 
  

  const stretchedOpacity = pow(opacity, float(3.0));
  const engineColor = pow(abs(accumulator), float(30.0));
  const finalColor = add(
    vec3(engineColor).mul(vec3(10.0, 0.3, 0.0)),
    vec3(0.0, 0.0, 0.0)
  ).mul(float(24.0));
  ...
  const finalColorWithOpacity = vec4(finalColor, mul(stretchedOpacity, 1.0));
  ...
  return {
    key: uTime.id,
    colorNode: finalColorWithOpacity,
  };
}
